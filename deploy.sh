...
# Find node/npm via absolutte stier — nvm virker ikke i non-interactive shell
NODE_PATH=$(find /root/.nvm/versions/node -name "node" -type f 2>/dev/null | sort -V | tail -1)
if [ -z "$NODE_PATH" ]; then
  log "FEJL: Kan ikke finde node i /root/.nvm"
  exit 1
fi
...
cd "$APP_DIR/frontend"
npm install --quiet
"$NODE_DIR/npx" vite build
log "Frontend OK"

# 5. Genstart
log "PM2: genstarter..."
"$NODE_DIR/pm2" restart kvitto-api || pm2 restart kvitto-api
log "PM2 OK"
